...
export default function CreateSpaceModal({
  ...
  const handleNext = () => {
    if (selectedType) {
      if (user?.role === "STUDIO_OWNER") {
        window.location.href = "/setup/studio";
      } else {
        window.location.href = `/auth?mode=register&role=STUDIO_OWNER`;
      }
    }
  };
  ...
  return (
    ...
    >
      <div className="bg-white rounded-2xl flex flex-col shadow-2xl w-[95%] md:w-[80%] lg:w-[60%] mx-4 py-8 px-6 md:py-12 md:px-8 relative animate-fadeIn max-h-[90vh] overflow-y-auto">
        {/* Close Button */}
        <button
          onClick={onClose}
        ...
        <button
          ...
          className={`w-full md:w-fit py-4 md:py-5 px-6 md:px-40 self-center md:self-end rounded-lg font-semibold text-white transition-all duration-300 ${selectedType
              ? "bg-black hover:bg-zinc-700 cursor-pointer"
